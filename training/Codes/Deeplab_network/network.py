...
class ResNet_segmentation(object):
	...
	def build_panoptic_decoder(self, encoding):
		...
		with tf.variable_scope('decoder') as scope:
			...
			outputs_s=tf.image.resize_images(outputs_s,(d_up[1]*2,d_up[2]*2))
			c=[]
			c.append(outputs_s)
			...
			outputs_i=tf.image.resize_images(outputs_i,[d_up[1]*2,d_up[2]*2])
			c=[]
			c.append(outputs_i)
	...
	def _depthwise_separable_conv2d(self, x, kernel_size, num_o, stride, name,dilations, biased=False):
		...
		with tf.variable_scope(name) as scope:
			...
			if biased:
				...
				o = tf.nn.bias_add(o, b)
			return o
	def _conv2d_tranpose(self, x, kernel_size, num_o, stride, name, biased=False):
		"""
		transpose conv2d without BN or relu.
		"""
		num_x = x.shape[self.channel_axis].value
		inp_w =x.shape[1]
		with tf.variable_scope(name) as scope:
			w = tf.get_variable('weights', shape=[kernel_size, kernel_size, num_o, num_x])
			s = [1, stride, stride, 1]
			o_s=inp_w*stride
			o = tf.layers.conv2d_transpose(x, num_x,kernel_size, stride, padding='SAME',name='t_conv2d')

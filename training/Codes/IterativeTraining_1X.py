...
def IterateTraining(args):
    ...
    if args.chop_data == 'True':
        ...
        classEnumHR=np.ones([classNum_HR,1])*args.aug_HR
...
def train_net(training_args,dirs):
    ...
    call(['python3', dirs['codedir'] +'/Codes/Deeplab_network/main.py', '--option', 'train',
        ...
        '--print_color', str(training_args['print_color'])])

def check_model_generation(dirs):
    modelsCurrent=os.listdir(dirs['basedir'] + dirs['project'] + dirs['modeldir'])
    gens=list(modelsCurrent)
    gens2 = []
    for gen in gens:
        gens2.append(int(gen))

    modelOrder=np.sort(gens2)[::-1]
    # idx = np.max(gens)

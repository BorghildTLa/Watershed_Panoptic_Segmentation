...
from skimage.morphology import binary_dilation,diamond,watershed,disk,extrema
...
class Model(object):
	...
	def predict(self):
		...
		for step in range(self.conf.test_num_steps):
			...
			if step>=0:
				...
				plt.subplot(132),plt.imshow(majority_vote[0,:,:]*50)
				#plt.subplot(223),plt.imshow(majority_vote)
				plt.subplot(133),plt.imshow(imb[0,:,:,:])
				plt.title(str(step))
				plt.show()
			'''
	...
	def predict_setup(self):
		...
		with tf.name_scope("create_inputs"):
			reader = ImageReader(
				...
				(self.conf.input_height, self.conf.input_width),
				False, # no data-aug
				False, # no data-aug
		...
		majority_map,centers=tf.py_func(self.get_majority_votes, [raw_output_up_a,raw_output,raw_output_up_c], [tf.uint8,tf.bool])
	...
	def get_majority_votes(self,dt_map,sem_map,center_map):
		...
		markers = label(center_map[0,:,:,0]>0.5)[0]
		#markers = label(cv2.adaptiveThreshold((center_map[0,:,:,0]*1.5).astype('uint8'),255,cv2.ADAPTIVE_THRESH_MEAN_C,cv2.THRESH_BINARY,251,0))[0]
		#markers=extrema.h_maxima(center_map[0,:,:,0],0.5)

		lab = watershed(-1*dt_map[0,:,:,0],markers,mask=dt_map[0,:,:,0]>0,watershed_line=True)

		lab[binary_dilation(lab==0,disk(3))]=0
		...
		pred_out[0,s[0],s[1]]=1


		#return (center_map[0,:,:,0]>0.1).astype('uint8')
		return pred_out,markers>0
